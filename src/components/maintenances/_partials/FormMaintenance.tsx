...
const FormMaintenance = ({
    ...
    const tireChange = (index, field, value) => {
    ...
    const markTireForReplacement = async (index, isNew) => {
        if (isNew) {
            ...
            if (tireToRemove && tireToRemove.id) {
                try {
                    ...
                    await VehicleTiresService.markTireForReplacement(tireToRemove.id, data)

                    ToastService.success("Pneu marcado para substituição com sucesso!");

                    // const updatedTires = tires.filter((_, i) => i !== index);
                    // onNewTiresChange(updatedTires);
                } catch (error) {
                    console.error("Erro ao marcar pneu:", error);
                }
            } else {
                console.error("Pneu não encontrado");
            }
        }
    };

    const removeTire = (index: number) => {
        // Cria uma cópia do array newTires
        const updatedTires = [...newTires];

        // Remove o item no índice especificado
        updatedTires.splice(index, 1);

        // Atualiza o estado newTires
        setNewTires(updatedTires);
    };

    const vehicleSelect = (id) => {
    ...
    return (
        <div>
            <div className="max-w-full mx-auto p-6 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
                ...
                <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
                        ...
                        <div className="flex-1">
                            ...
                            <VehicleToSelect defaultVehicleId={defaultVehicleId}
                                onSelect={(id) => {
                                    ...
                                    vehicleSelect(id); // Atualiza o estado local também, caso precise
                                }} onMileageChange={mileageChange} />
                        </div>
                        <div className="flex-1">
                    ...
                    <div>
                        ...
                        {(tires || []).map((tire, index) => (
                            <TireItem key={index} tire={tire} index={index} onRemove={() => markTireForReplacement(index, false)} />
                        ))}
                        ...
                        {showInputs && newTires.map((tire, index) => (
                            <NewTireForm
                                ...
                                index={index}
                                onChange={tireChange}
                                onRemove={() => removeTire(index, true)}
                            />
                        ))}
                    </div>
